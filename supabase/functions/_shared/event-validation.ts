 ...
 * The validation seam.
 *
 * n8n posts to /ingest rather than writing to the database directly, and this is the reason
 * why: one versioned place that rejects a malformed payload at the door. Schema drift
 * upstream — a renamed form field, a changed webhook shape — is the documented way this
 * pipeline dies quietly, so a bad payload must produce a loud 400 rather than a row full of
 * nulls that reads as a healthy event.
 *
 * It lived in `ingest/validate.ts` until Lead Recovery (0010), and moved here unchanged the
 * day Arc started *emitting* events as well as receiving them. The engine writes
 * `sms_sent`, `message_delivered`, `lead_booked` and the rest from inside the twilio,
 * lead-intake and dispatch functions, and those rows must be identical in shape and
 * discipline to the ones an outside workflow posts. An internal writer with its own looser
 * path would be a second door into the only append-only table in the system — and the
 * first event with a defaulted `occurred_at` or an unrecognised type would make every
 * figure derived from the log arguable.
 *
 * So: there is one validator, one list of event types, one set of defaults, and everything
 * that writes an event goes through it. `ingest/validate.ts` re-exports this file so the
 * public boundary keeps its documented name.
 */
...
export const EVENT_TYPES = [
  ...
  'lead_qualified',
  'handoff_requested',
  /* ── added by Lead Recovery (0010) ──
     five things the execution layer can prove that an observing adapter never could.
     deliberately the minimum: every other state the engine passes through is already
     derivable from the events above, and an event type that adds no provable claim is a
     column in the log nobody reads. */
  // the provider said it arrived, or said it did not. "sms_sent" has only ever meant
  // "we handed it to twilio"; delivery is a separate fact that arrives later, by callback.
  'message_delivered',
  'message_failed',
  // the lead turned into work. the only conversion claim this product makes.
  'lead_booked',
  // this contact must not be messaged again, and why.
  'lead_suppressed',
