...
export function supabaseStore(db: Db): EngineStore {
  ...
  return {
     ...
     */
    async claimActions(limit, worker, _nowIso, leaseSeconds = 120) {
      const { data, error } = await db.rpc('claim_scheduled_actions', {
        p_limit: limit,
        p_worker: worker,
    ...
    async completeAction(actionId, status, error, nowIso) {
      const { error: updateError } = await db
        .from('scheduled_actions')
        .update({ status, last_error: error, completed_at: nowIso, locked_at: null, locked_by: null })
        .eq('id', actionId);
      if (updateError) fail('action complete', updateError);
    },

    async rescheduleAction(actionId, runAt, error) {
      const { error: updateError } = await db
        .from('scheduled_actions')
        .update({ status: 'pending', run_at: runAt, last_error: error, locked_at: null, locked_by: null })
        .eq('id', actionId);
      if (updateError) fail('action reschedule', updateError);
    },
